...
large_int* mult_kar_eqsize_s(large_int* a, large_int* b);
...
large_int* mult_kar_s(large_int* a, large_int* b) {
  int n;
...
if (debug) printf("Calling mult_kar_s\n");
...
if (debug) printf("Calling mult_kar_eqsize_s\n");
...
if (debug) printf("d0 ");
  a1_mult_b1=mult_kar_eqsize_s(&a1,&b1);
// if (debug) { printf("a1*b1 : "); print_hex(a1_mult_b1); }
  a0_mult_b0=mult_kar_eqsize_s(&a0,&b0);
// if (debug) { printf("a0*b0 : "); print_hex(a0_mult_b0); } 
  a1pa0_mult_b1pb0=mult_kar_eqsize_s(a1_plus_a0,b1_plus_b0);
if (debug) {
  printf("a0: "); print_hex(&a0);
  printf("a1: "); print_hex(&a1);
  printf("b0: "); print_hex(&b0);
  printf("b1: "); print_hex(&b1);
  printf("rec - a1*b1 : "); print_hex(a1_mult_b1);
  printf("gmp - a1*b1 : "); temp=mult_gmp(&a1,&b1); print_hex(temp); free(temp->int_array); free(temp);
  printf("rec - a0*b0 : "); print_hex(a0_mult_b0);
  printf("gmp - a0*b0 : "); temp=mult_gmp(&a0,&b0); print_hex(temp); free(temp->int_array); free(temp);
  printf("rec - (a1+a0)*(b1+b0) : ");
  print_hex(a1pa0_mult_b1pb0);
  large_int* temp2=add_gmp(&a1,&a0);
  large_int* temp3=add_gmp(&b1,&b0);
  temp=mult_gmp(temp2,temp3);
  printf("gmp - (a1+a0)*(b1+b0) : ");
  print_hex(temp);
  free(temp2->int_array);
  free(temp3->int_array);
  free(temp->int_array);
  free(temp3);
  free(temp2);
  free(temp);
  ...
  if (debug && compare(a1_mult_b1,a1pa0_mult_b1pb0)>0) {
    if (debug) printf("Noticed that a1b1 > (a1+a0)(b1+b0), this should not be\n");
    temp=sub_s(a1_mult_b1,a1pa0_mult_b1pb0);
    temp->sign=-1;
  }
  else {
    if (debug) printf("Calling sub_s -a1b1 , first_size=%d, second_size=%d\n",a1pa0_mult_b1pb0->size,a1_mult_b1->size);
    temp=sub_s(a1pa0_mult_b1pb0,a1_mult_b1);
    temp->sign=1;
    if (debug) print_hex(temp);
  }
  if (debug && compare(temp,a0_mult_b0)<0) {
    if (debug) printf("Noticed that a0b0 > reminder, should not be");
    a1b0_plus_a0b1=sub_s(a0_mult_b0,temp);
    if (temp->sign==-1) { if (debug) printf("but we're good\n"); }
    else { printf("We have a problem!\n"); a1b0_plus_a0b1->sign=-1; }
  }
  else {
    if (debug) printf("calling sub_s -a0b0: ");
    a1b0_plus_a0b1=sub_s(temp,a0_mult_b0);
  ...
  free(b1_plus_b0);
  free(a1b0_plus_a0b1);
  if (debug) { printf("About to exit kmul_eqsize. Result is: \n"); print_hex(result); }
